import type { AuditStatusResponse } from "@/lib/contracts";
...
export function buildAuditMarkdown(report: AuditStatusResponse): string {
  ...
  if (!report.issues.length) {
    ...
    for (const issue of report.issues) {
      ...
      for (const step of issue.reproSteps) lines.push(`  - ${step}`);
      lines.push("- Verification Steps:");
      for (const step of issue.verificationSteps) lines.push(`  - ${step}`);
      if (issue.evidenceLinks.length) {
        lines.push("- Evidence Links:");
        for (const link of issue.evidenceLinks) lines.push(`  - ${link}`);
      }
    }
  }
