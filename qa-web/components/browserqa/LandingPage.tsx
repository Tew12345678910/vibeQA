...
/* ─── Typewriter ──────────────────────────────────────────────── */
const WORDS = ["understand your code.", "ship with confidence.", "learn as you build.", "vibe smarter."];
...
/* ─── Lesson callout ──────────────────────────────────────────── */
function Lesson({ n, title, body }: { n: string; title: string; body: string }) {
  ...
  return (
    ...
    >
      <div className="flex items-center gap-2">
        <span className={`rounded px-2 py-0.5 text-xs font-bold text-slate-950 ${badgeColor}`}>
          {priority}
        </span>
        <span className={`text-xs font-semibold ${priorityColor}`}>{category}</span>
      </div>
...
/* ─── Code snippet block ──────────────────────────────────────── */
function CodeBlock({ lines }: { lines: Array<{ code: string; annotation?: string; highlight?: boolean }> }) {
  return (
      ...
      <div className="space-y-0">
        {lines.map((l, i) => (
          ...
          >
            ...
            <span className={l.highlight ? "text-red-300" : "text-slate-400"}>{l.code}</span>
            {l.annotation && (
              <span className="ml-auto text-xs text-amber-400/70 italic">{l.annotation}</span>
            )}
          </div>
        ))}
      </div>
...
function TerminalMockup() {
  const lines = [
    { delay: 0,    text: "$ vibeqa analyze github.com/my-app", color: "text-slate-300" },
    { delay: 400,  text: "  ✓ Fetching repo tree (243 files)...", color: "text-slate-400" },
    { delay: 800,  text: "  ✓ Static checks: endpoints, auth, validation", color: "text-slate-400" },
    { delay: 1200, text: "  ✓ AI report generated (GPT-4.1)", color: "text-emerald-400" },
    { delay: 1600, text: "  ↳ Browser agent launched at https://my-app.com", color: "text-blue-400" },
    { delay: 2000, text: "  ✓ 18 issues found  ·  3 P0  ·  8 P1  ·  7 P2", color: "text-amber-400" },
    { delay: 2400, text: "  ✓ Each issue includes: why it matters + fix steps", color: "text-emerald-400" },
  ];
  ...
  return (
    <div className="w-full overflow-hidden rounded-2xl border border-slate-700/60 bg-slate-950 shadow-2xl shadow-black/60">
      ...
      <div className="space-y-1.5 p-6 font-mono text-sm">
        {lines.map((l, i) => (
          <p key={i} className={[l.color, "transition-all duration-500", i < shown ? "opacity-100 translate-x-0" : "opacity-0 -translate-x-4"].join(" ")}>
            {l.text}
          </p>
        ))}
        {shown >= lines.length && <p className="mt-2 animate-pulse text-slate-500">█</p>}
      </div>
    </div>
  );
}
    ...
    <div className="w-full overflow-hidden rounded-2xl border border-slate-700/60 bg-slate-950 shadow-2xl shadow-black/60">
      <div className="flex items-center gap-2 border-b border-slate-800 bg-slate-900/80 px-4 py-3">
        ...
        <div className="ml-2 flex flex-1 items-center gap-2 rounded-md border border-slate-700 bg-slate-800/60 px-3 py-1">
          ...
          <span className="text-xs text-slate-400">vibeqa.app / issues / P0-001</span>
        </div>
      </div>
      <div className="space-y-4 p-5">
        <div className="flex items-center gap-2">
          <span className="rounded bg-red-500 px-2 py-0.5 text-xs font-bold text-white">P0</span>
          <span className="text-sm font-semibold text-slate-100">Missing CSRF protection on /api/auth/login</span>
        </div>
        <div className="space-y-1">
          <p className="text-xs font-semibold uppercase tracking-wider text-slate-500">Why it matters</p>
          <p className="text-xs leading-relaxed text-slate-300">An attacker can craft a page that silently submits a login form on behalf of a visitor, hijacking their session without any interaction.</p>
        </div>
        <div className="space-y-1">
          <p className="text-xs font-semibold uppercase tracking-wider text-slate-500">Evidence — app/api/auth/login/route.ts:12</p>
          <div className="rounded-lg border border-slate-800 bg-slate-900 p-3 font-mono text-xs text-red-300">
            ...
            <p className="pl-4 text-slate-400">{"// ← no origin / csrf token check"}</p>
            <p className="pl-4">{"const { email, password } = await req.json();"}</p>
          </div>
        </div>
        <div className="space-y-2">
          <p className="text-xs font-semibold uppercase tracking-wider text-slate-500">Fix steps</p>
          {["Verify Origin / Referer header server-side", "Add a CSRF token signed with your session secret", "Reject requests where Origin doesn't match your domain"].map((s, i) => (
...
export function LandingPage() {
  ...
  return (
    <div className="min-h-screen overflow-x-hidden bg-slate-950 text-slate-100">
      ...
      <nav className="fixed inset-x-0 top-0 z-40 border-b border-slate-800/60 bg-slate-950/80 backdrop-blur-md">
        <div className="mx-auto flex h-14 max-w-6xl items-center justify-between px-6">
          ...
          <div className="hidden items-center gap-6 text-sm text-slate-400 sm:flex">
            <a href="#the-blind-spot" className="transition-colors hover:text-slate-200">The blind spot</a>
            <a href="#what-you-learn" className="transition-colors hover:text-slate-200">What you learn</a>
            <a href="#issue-types" className="transition-colors hover:text-slate-200">Issue types</a>
          </div>
      ...
      <section className="relative flex min-h-screen flex-col items-center justify-center px-6 pt-14">
        ...
        >
          ...
          <p className="mx-auto mb-8 max-w-2xl text-lg leading-relaxed text-slate-400">
            ...
            <span className="text-slate-200">source code</span>, one operates a real browser on your{" "}
            <span className="text-slate-200">live site</span> — then explains exactly what's wrong and{" "}
            <span className="text-emerald-400">teaches you why it matters</span>.
          </p>

          <div className="flex flex-col items-center gap-3 sm:flex-row sm:justify-center">
            <Link href="/projects/new">
              <Button size="lg" className="gap-2 bg-emerald-500 px-8 text-slate-950 shadow-lg shadow-emerald-500/25 hover:bg-emerald-400">
                Scan my repo — it&apos;s free <ArrowRight className="h-4 w-4" />
              </Button>
            </Link>
            <a href="#the-blind-spot">
              <Button variant="outline" size="lg" className="border-slate-700 bg-transparent text-slate-300 hover:border-slate-500 hover:bg-slate-800/60 hover:text-white">
                What am I missing?
              </Button>
            </a>
          </div>

          <div className="mt-16 grid grid-cols-3 gap-8 border-t border-slate-800/60 pt-8">
            <div className="flex flex-col items-center gap-1">
              <span className="text-3xl font-bold text-white">2</span>
              <span className="text-sm text-slate-400">agents — code + browser</span>
            </div>
            <div className="flex flex-col items-center gap-1">
              <span className="text-3xl font-bold text-white">20+</span>
              <span className="text-sm text-slate-400">quality checks explained</span>
            </div>
            <div className="flex flex-col items-center gap-1">
              <span className="text-3xl font-bold text-white">0</span>
              <span className="text-sm text-slate-400">config files needed</span>
            </div>
          </div>
        </div>
      </section>
      ...
      {/* ── The blind spot ── */}
      <section id="the-blind-spot" className="relative mx-auto max-w-6xl px-6 pb-32">
        <div
          ref={blindspotReveal.ref}
          className={[
            "transition-all duration-700",
            blindspotReveal.visible ? "opacity-100 translate-y-0" : "opacity-0 translate-y-8",
          ].join(" ")}
        >
          <div className="mb-10 max-w-2xl">
            ...
            <h2 className="mb-4 text-3xl font-bold text-white sm:text-4xl">
              ...
              <span className="text-amber-400">But who checks the code AI writes?</span>
            </h2>
            <p className="text-slate-400 leading-relaxed">
              Vibe coding — using AI assistants to generate features at speed — is incredibly powerful.
              But AI models are optimised to make code that <em className="text-slate-300 not-italic font-medium">works</em>, not code that is
              secure, observable, and production-ready. Two entire coverage gaps are almost always left open.
            </p>
          </div>
          ...
          <div className="grid gap-6 lg:grid-cols-2">
            ...
            <div className="rounded-2xl border border-red-500/20 bg-red-500/5 p-6">
              <div className="mb-4 flex items-center gap-3">
                ...
                <div>
                  <p className="font-semibold text-slate-100">Gap 1 — Static code quality</p>
                  <p className="text-xs text-slate-500">What the AI never mentions</p>
                </div>
              </div>
              <div className="space-y-3 text-sm text-slate-400">
                <p>AI assistants generate route handlers, auth flows, and API endpoints — but they routinely skip:</p>
                ...
                <p className="pt-1 text-slate-500 italic text-xs">These aren't bugs — the code runs fine. They're the gaps that let attackers in and make production incidents impossible to debug.</p>
              </div>
            </div>
            ...
            <div className="rounded-2xl border border-blue-500/20 bg-blue-500/5 p-6">
              <div className="mb-4 flex items-center gap-3">
                ...
                <div>
                  <p className="font-semibold text-slate-100">Gap 2 — Runtime behaviour</p>
                  <p className="text-xs text-slate-500">What only a real browser finds</p>
                </div>
              </div>
              <div className="space-y-3 text-sm text-slate-400">
                <p>Code review tells half the story. A browser agent finds the other half by actually using your app:</p>
                ...
                <p className="pt-1 text-slate-500 italic text-xs">No static analyser can see these. You need a real browser to run the flows a user would actually take.</p>
              </div>
            </div>
          </div>
        </div>
      ...
      <section className="mx-auto max-w-6xl px-6 pb-32">
        <div className="mb-10 text-center">
          <p className="mb-2 text-sm font-semibold uppercase tracking-widest text-emerald-400">See it in action</p>
          <h2 className="text-3xl font-bold text-white sm:text-4xl">Both agents, running in parallel</h2>
        ...
        <div className="grid gap-6 lg:grid-cols-2">
          ...
          <div>
            <p className="mb-3 flex items-center gap-2 text-sm font-medium text-slate-400">
              <MonitorCheck className="h-4 w-4 text-blue-400" /> Issue detail — with education built in
            </p>
            <IssueDetailMockup />
          </div>
        </div>
        ...
        <div className="grid gap-14 lg:grid-cols-2 lg:items-start">
          ...
          <div>
            ...
            <h2 className="mb-3 text-3xl font-bold text-white sm:text-4xl">
              Every finding is a lesson,<br />
              <span className="text-violet-400">not just a ticket.</span>
            </h2>
            <p className="mb-8 text-slate-400 leading-relaxed">
              Most QA tools hand you a bug list. VibeQA explains the <em className="not-italic font-medium text-slate-200">why</em> behind every
              issue — so you ship the fix and understand the pattern well enough not to repeat it.
            </p>
            ...
            <div className="rounded-2xl border border-slate-800 bg-slate-900/60 p-5">
              ...
              <CodeBlock
                lines={[
                  ...
                  { code: "  const { email, password } = await req.json();", highlight: true, annotation: "← no validation" },
                  { code: "  const user = await db.users.findFirst({" },
                  { code: "    where: { email }" },
                  { code: "  });" },
                  { code: "  // ... auth logic", highlight: true, annotation: "← no rate limit" },
                  { code: "}" },
                ]}
              />
      ...
      <section className="mx-auto max-w-6xl px-6 pb-32">
        <div className="mb-12 text-center">
          <p className="mb-2 text-sm font-semibold uppercase tracking-widest text-emerald-400">How it works</p>
          <h2 className="text-3xl font-bold text-white sm:text-4xl">Paste a URL. Get a curriculum.</h2>
        </div>
        <div className="relative grid gap-12 md:grid-cols-3">
          ...
          {[
            ...
            return (
              ...
              >
                <div className="flex h-10 w-10 items-center justify-center rounded-full border border-emerald-500/40 bg-emerald-500/10 text-sm font-bold text-emerald-400">{n}</div>
                <div>
                  <h3 className="mb-1.5 text-base font-semibold text-slate-100">{title}</h3>
                  <p className="text-sm leading-relaxed text-slate-400">{body}</p>
                </div>
              </div>
            );
      ...
      <section className="relative overflow-hidden px-6 pb-32">
        ...
        <div className="relative z-10 mx-auto max-w-3xl text-center">
          ...
          <h2 className="mb-4 text-3xl font-bold text-white sm:text-4xl">
            The fastest way to level up<br />
            <span className="text-emerald-400">your vibe-coded projects.</span>
          </h2>
          <p className="mx-auto mb-8 max-w-xl text-slate-400">
            Paste your GitHub URL. In under two minutes, get a full code + browser audit —
            with explanations that make you a better builder, not just a faster one.
          </p>
          ...
          <div className="mb-10 grid gap-3 sm:grid-cols-3">
            {[
              ...
              <div key={text} className="flex items-center justify-center gap-2 text-sm text-slate-300">
          ...
          <Link href="/projects/new">
            <Button size="lg" className="gap-2 bg-emerald-500 px-10 text-slate-950 shadow-lg shadow-emerald-500/25 hover:bg-emerald-400">
              Scan my repo — it&apos;s free <ArrowRight className="h-4 w-4" />
            </Button>
          </Link>
        </div>
      ...
      <footer className="border-t border-slate-800/60 px-6 py-8 text-sm text-slate-500">
        <div className="mx-auto flex max-w-6xl flex-col items-center gap-3 sm:flex-row sm:justify-between">
          ...
          <div className="flex items-center gap-4">
            ...
            <Link href="/projects/new" className="transition-colors hover:text-slate-300">Get started</Link>
          </div>
        </div>
      </footer>
    </div>
