...
function toRunStatus(rawStatus: unknown): RunStatus {
  ...
  if (["completed", "finished", "succeeded", "success", "done"].includes(normalized)) {
    return "completed";
  }
...
function parsePageResults(raw: Record<string, unknown>, baseUrl: string): PageResult[] {
  const source = readField<unknown[]>(raw, ["pageResults", "results", "pages"]) ?? [];
  ...
  return source
    ...
    .map((row) => {
      ...
      return {
        ...
        viewportKey,
        finalUrl: String(row.finalUrl ?? row.fullUrl ?? `${baseUrl.replace(/\/$/, "")}${route}`),
        title: String(row.title ?? ""),
        status,
        signals: {
          ctaAboveFold: typeof signalsRaw.ctaAboveFold === "boolean" ? signalsRaw.ctaAboveFold : null,
          mobileHorizontalScroll:
            typeof signalsRaw.mobileHorizontalScroll === "boolean"
              ? signalsRaw.mobileHorizontalScroll
              : null,
          navWorks: typeof signalsRaw.navWorks === "boolean" ? signalsRaw.navWorks : null,
          formLabelingOk: typeof signalsRaw.formLabelingOk === "boolean" ? signalsRaw.formLabelingOk : null,
        },
        evidence: {
          screenshots,
  ...
  return source
    ...
    .map((row) => {
      const severity = ["high", "medium", "low"].includes(String(row.severity))
        ...
        : "low";
      const category = ["functional", "usability", "accessibility", "security", "content"].includes(
        String(row.category),
      )
...
function parseArtifacts(raw: Record<string, unknown>, pageResults: PageResult[], issues: Issue[]): Artifact[] {
  const direct = (readField<unknown[]>(raw, ["artifacts", "evidenceLinks"]) ?? [])
    .map((item) => asRecord(item))
    .map((row) => ({
...
function deriveProgress(status: RunStatus, pageResults: PageResult[]): AuditProgress {
  const totalChecks = pageResults.length;
  const completedChecks = pageResults.filter((item) => ["ok", "warning", "error"].includes(item.status)).length;
...
function deriveSummary(
  ...
  const failCount = pageResults.filter((item) => item.status === "error").length;
  const passCount = pageResults.filter((item) => item.status === "ok").length;

  return {
    baseUrl,
    pagesAudited: Number(rawSummary.pagesAudited ?? pageResults.length),
    viewports: ["desktop", "mobile"],
    passCount: Number(rawSummary.passCount ?? passCount),
    failCount: Number(rawSummary.failCount ?? failCount),
    highRiskCount: Number(rawSummary.highRiskCount ?? issues.filter((item) => item.severity === "high").length),
    keyFindings: (Array.isArray(rawSummary.keyFindings) ? rawSummary.keyFindings : issues.map((item) => item.title))
...
export function emptyAuditSummary(baseUrl: string): AuditSummary {
  return {
    baseUrl,
    pagesAudited: 0,
    viewports: defaultViewports.map((viewport) => viewport.key),
    passCount: 0,
    failCount: 0,
    highRiskCount: 0,
    keyFindings: [],
...
export function mapCloudAuditPayload({ raw, baseUrl }: CloudMapInput): CloudMapOutput {
