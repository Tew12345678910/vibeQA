...
import { emptyAuditSummary } from "../cloud/mapper";
...
export async function createAuditRun(input: AuditRequest): Promise<{ auditId: string; status: RunStatus }> {
  await ensureSchema();
  const sql = getSql();
...
export async function markRunFailed(auditId: string, message: string): Promise<void> {
  await ensureSchema();
  const sql = getSql();
  const ts = nowMs();
...
export async function updateRunWithCloudSnapshot(args: {
  ...
  const sql = getSql();
  const ts = nowMs();
  const finishedAt = ["completed", "failed", "canceled"].includes(args.status) ? ts : null;
  ...
  for (const item of args.pageResults) {
    await sql`
      INSERT INTO audit_page_results (
        audit_id,
        route,
        viewport_key,
        status,
...
export async function getAuditStatusResponse(auditId: string): Promise<AuditStatusResponse | null> {
  await ensureSchema();
  const sql = getSql();
  ...
  const [pages, issues, artifacts] = await Promise.all([
    sql<AuditPageRow[]>`
      ...
      ORDER BY route ASC, viewport_key ASC
    `,
    sql<AuditIssueRow[]>`
      SELECT issue_json
      FROM audit_issues
  ...
  return {
    ...
    progress,
    summary,
    pageResults: pages.map((entry) => entry.result_json),
    issues: issues.map((entry) => entry.issue_json),
    artifacts: {
      links: artifacts.map((entry) => ({
        kind: entry.kind,
        url: entry.url,
...
export async function listAuditStatusResponses(args: {
  ...
  const nextCursor = hasNext ? asMs(slice[slice.length - 1]?.created_at ?? null) : null;
