...
function resolveBaseUrl(): string {
  ...
  if (!raw) throw new Error("CLOUD_BROWSER_API_BASE_URL is required");
  return raw.replace(/\/$/, "");
}

function resolveHeaders(): Record<string, string> {
  const apiKey = process.env.CLOUD_BROWSER_API_KEY;
  if (!apiKey) throw new Error("CLOUD_BROWSER_API_KEY is required");
  return {
...
export const cloudBrowserClient: CloudBrowserClient = {
  async startAudit(payload: AuditRequest): Promise<CloudStartResponse> {
    ...
    const externalRunId = readField<string>(raw, [
      "externalRunId",
      "id",
      "runId",
      "auditId",
    ...
    return { externalRunId, status };
  },

  async getAudit(externalRunId: string): Promise<CloudAuditPayload> {
    return requestJson(`/audits/${externalRunId}`, { method: "GET" });
  },

  async cancelAudit(externalRunId: string): Promise<void> {
    await requestJson(`/audits/${externalRunId}/cancel`, { method: "POST" });
  },
};
