...
import { cloudBrowserClient } from "../cloud/client";
import { mapCloudAuditPayload } from "../cloud/mapper";
import {
  cancelRun,
  createAuditRun,
...
import { normalizeRoutes, validateHostedHttpsUrl } from "../utils/urlSafety";
...
export async function startAudit(input: AuditRequest): Promise<{ auditId: string; status: RunStatus }> {
  ...
  try {
    ...
    const message = error instanceof Error ? error.message : "Failed to start cloud audit";
...
async function syncAuditSnapshot(auditId: string): Promise<void> {
  ...
  if (!runRow || !runRow.external_run_id) {
    return;
  }

  if (!shouldSyncRun(runRow)) {
    return;
  }
  ...
  try {
    ...
    const message = error instanceof Error ? error.message : "Cloud status sync failed";
    await markRunFailed(auditId, message);
  }
}

export async function getAudit(auditId: string): Promise<AuditStatusResponse | null> {
