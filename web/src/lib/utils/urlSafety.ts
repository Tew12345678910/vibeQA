...
export function validateHostedHttpsUrl(rawUrl: string): { valid: boolean; message?: string } {
  try {
    ...
    if (isPrivateOrLocalHostname(url.hostname)) {
      return { valid: false, message: "Private or localhost URLs are not allowed in hosted mode" };
    }
    return { valid: true };
  } catch {
